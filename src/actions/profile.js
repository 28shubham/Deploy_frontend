...
export function fetchUserProfile(usedId) {
  return (dispatch) => {
    let url = `https://deployment-backend-webapp.onrender.com/user/getProfile?user=${usedId}`;
...
export function updateUserProfile(field, data) {
  return (dispatch) => {
    let url = `https://deployment-backend-webapp.onrender.com/user/updateProfile?user=${field}`;
    let token = localStorage.getItem("token");
    let options = {
      method: "POST",
      headers: {
